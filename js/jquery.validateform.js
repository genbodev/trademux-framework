...
jQuery.iFormValidate = {
	build : function(user_options)
	{
		...
		return $(this).each(
			...
			if(options.validCheck){
				var $inputs = $(this).find(":input").filter(":not(:submit)").filter(":not(:checkbox)").filter(":not(.novalid)").filter(":not(:radio)");
			}else{
				var $inputs = $(this).find(":input").filter(":not(:submit)").filter(":not(:checkbox)").filter(":not(:radio)");
			}
			$radios = $(this).find(":input").filter(":radio");
			//catch the submit
			$(this).submit(function(){
				...
				$radios.filter(".is_required").each( function (i) 
				{
				...
				if(!isValid){
					if (options.error_msg) alert(options.error_msg);
					return false;
				}
				if(options.ajax){
					var data = {};
			...
			$radios.each( function (i)
			{
				...
				{
					$('input[name="'+$(this).attr('name')+'"]').removeClass("invalid").addClass("valid");
					$('input[name="'+$(this).attr('name')+'"]').parent().removeClass("invalid").addClass("valid");
				});
				if ($(this).attr('checked')) $('input[name="'+$(this).attr('name')+'"]').removeClass("invalid").addClass("valid");
			});
	...
	validateForm : function($inputs, $radios)
	{
